...
-module(emqx_plugin_test_broadcast).
...
handle_call(Req, _From, State) ->
  io:fwrite("Unexpected request: ~p", [Req]),
	{reply, ignored, State}.

handle_cast(Msg, State) ->
  io:fwrite(error, "Unexpected msg: ~p", [Msg]),
	{noreply, State}.
...
handle_info(Info, State) ->
    io:fwrite(error, "Unexpected info: ~p", [Info]),
	{noreply, State}.
...
send_advertise(#state{gwid = GwId, sock = Sock, port = Port,
    ...
    io:format("send_advertise: GwId:~p,Duration:~p ~n",[GwId, Duration]).
